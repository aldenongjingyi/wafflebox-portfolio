    ...
    <section id="services" className="py-32 px-6 max-w-6xl mx-auto">
      <div className="mb-16">
        ...
        <h2 className="text-4xl md:text-5xl font-black tracking-tight">
          Full stack studio.
          <br />
          <span className="text-black/30">No middlemen.</span>
        </h2>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-px bg-black/10">
        {services.map((s) => (
          <div
            key={s.number}
            className="bg-white p-10 flex flex-col gap-6 group hover:bg-zinc-50 transition-colors duration-300"
          >
            <span className="text-black/30 text-xs font-mono tracking-widest">
              {s.number}
            </span>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {s.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-xs text-black/40 border border-black/10 px-3 py-1"
                >
                  {tag}
                </span>
              ))}
